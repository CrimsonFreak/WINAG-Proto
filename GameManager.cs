...
public class GameManager : MonoBehaviour
{
    ...
    void MapViewSelect(GameObject SectionLeader)
    {
        ...
        if (SelectedSection.MissionsList.Find(s => !s.IsValid) != null)
        {
            var cancelOrSend = SendButtonsContainer;
            cancelOrSend.GetComponentsInChildren<Button>()[0].onClick.AddListener(() => SelectedSection.CancelPendingMissions(false));
            cancelOrSend.GetComponentsInChildren<Button>()[1].onClick.AddListener(() => SelectedSection.SendPendingMissions());
    ...
    private void MoveMapUIElement(Ray ray)
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, LayerMask.GetMask("FOWRGB")))
        {
            ...
            Mission _mission = MovingObject.GetComponentInParent<Mission>();
            ...
            for (int i = 0; i < container.transform.parent.childCount; i++)
            {
                ...
                if (_mission.Type == MissionsTools.MissionTypes.Surveiller)
                {
    ...
    public void SceneViewUnselect()
    {
        if (SelectedUnit != null)
        {
            ...
            SelectedUnit.MainUiObject.GetComponentInChildren<Image>().color = SelectedUnit.BaseColor;
    ...
    public void NextTurn()
    {
        ...
        {
            Section.CancelPendingMissions(true);
            ...
            var _actionList = Section.MissionsList;
            foreach (var _mission in _actionList)
            {
                if (_mission.Passed) continue;
                else if (_mission.EndingTurn <= CurrentTurn - 1)
                {
                    Section.CurrentMission = _actionList[_mission.Number];
                    _mission.Passed = true;
                    _mission.TimelineObject.GetComponentInChildren<CursorChanger>().gameObject.SetActive(false);
        ...
        foreach (var Section in SectionList)
        {
            if (Section.IsOwned)
            {
                ...
                foreach (PlayerUnit Unit in Section.PlayerUnits)
                {
                    ...
                    {
                        if (resolvingPhaseInputs.timeIterator % 5 == 0) RunDetectionTest(Unit, mods, new ActionModifiers(ActionTypes.None));
                        continue;
                    }
